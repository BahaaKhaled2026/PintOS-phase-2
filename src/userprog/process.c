  ...
  if (kpage != NULL) 
    {
      ...
      if (success){
        *esp = PHYS_BASE;
        ...
        // Divide file_name into strings upon spaces and store in args array
        char *token;
        char *save_ptr;
        int i = 0;
        ...
        int numStrings=countStrings(args);
        void *stack_addresses[numStrings];
        for (int i = 0; i < numStrings; i++) {
            char *currentString = args[i];
            int stringLength = 0;
            while (*currentString != '\0') {
                stringLength++;
                currentString++;
            }
            totalCharacters += stringLength;
        }
        int aligns=0;
        int check=totalCharacters;
        while(check%4!=0){
          aligns++;
          check++;
        }
        ...
        for(int i=numStrings-1;i>=0;i--){
          *esp -= strlen(args[i]) + 1;
          ...
          stack_addresses[i] = *esp;
        }
        for(int i=0;i<aligns;i++){
          *esp -= 1;  
          *(uint8_t *)*esp = 0; 
        }


        // Push the addresses of the strings onto the stack
        for (int i = numStrings - 1; i >= 0; i--) {
            *esp -= sizeof(char *);
            *(char **)*esp = stack_addresses[i];
        }

        // Push the address of argv[0] onto the stack
        char **argv = *esp;
        *esp -= sizeof(char **);
        *(char ***)(*esp) = argv;
